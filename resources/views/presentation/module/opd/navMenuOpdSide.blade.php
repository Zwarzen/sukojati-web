@php

$opdProfile = isset($opd) ? $opd : $opdViaServicesProvider;
@endphp


<div id="navbar_main_opd" class="must-close mobile-offcanvas">


    <style>
        @media only screen and (max-width: 400px) {

            .title-teks-sub-opd-a {
                max-width: 200px !important;
                overflow: hidden;
            }

        }

        /* .must-close {
            background: #ffffff9e;
        } */

        /* .mobile-offcanvas-opd-inner{
            background: #ffffff !important;
            color: aqua !important;
        } */

        /* #navbar_main_opd_inner{
            background: #ffffff !important;
            color: aqua !important;
        } */
    </style>


    <div id="navbar_main_opd_inner" class="container-fluid mobile-offcanvas-inner scroller ">
        <div class="mobile-offcanvas-opd-inner">


            <div class="offcanvas-header">
                <button class="btn float-right btn-close"><i class="fas fa-times"></i> </button>
                <a class="navbar-brand" href="{{ url('/') }}">
                    <div style="display: flex; flex-direction:row;">
                        <img src="{{ asset('presentation/b-asset/img/logo_bwi_small.png') }}"
                            class="logo-beranda-img">
                        <div id="title-teks" class="logo-beranda">
                            <span class="title-teks-sub-opd-a">{{ $opdProfile->alias }}</span>
                            <span class="title-teks-sub-opd-b">Kabupaten Banyuwangi</span>
                        </div>
                        {{-- <span class="title-teks" style="color: #303030"> {{ $opd->alias }} </span> --}}
                    </div>
                </a>
            </div>
            <br>



            @include('presentation.module.opd.navMenuOpd')

        </div>



    </div>
</div>
