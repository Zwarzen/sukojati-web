@if (count($latestBannerOpdData) > 0)
    ...
    <div class="owl-carousel owl-theme" id="dt-popupinfo-list" >
        @foreach ($latestBannerOpd as $item)
            <?php $forward = strlen($item->link_forward) > 10 ? 'yes' : 'no'; ?>
            @php $img_link = $item->img_raw !== '' || $item->img_raw !== null ? url($base_link_media_popupinfo_raw . $item->img_raw) : asset('presentation/b-asset/img/lambang-daerah.png'); @endphp


            <a href="{{ $forward === 'yes' ? $item->link_forward : $img_link }}">
                <div class="banner-top-wrp">
                    {{-- <div style="
                        background:url('{{ $item->img_raw != '' || $item->img_raw != null ?
                        $base_link_media_popupinfo_raw . $item->img_raw :
                        asset('presentation/b-asset/img/lambang-daerah.png') }}');"
                        class="banner-top-item ">

                    </div> --}}

                    <img class="img-banner-top-item "
                        src="{{ $item->img_raw != '' || $item->img_raw != null? $base_link_media_popupinfo_raw . $item->img_raw: asset('presentation/b-asset/img/lambang-daerah.png') }}"
                        alt="">
