@if(isset($latestBeritaVideo) && $latestBeritaVideo->count() > 0)
<div style="">
  <div style="font-size:1.2rem; font-weight:600; margin-bottom:1rem">Video Terbaru<small
              ...
              <a href="{{ route('berita.video')."/".$item->slug }}" title="{{ $item->title }}">
                  ...
                  <div style="overflow:hidden; display:flex; flex-direction:column;  border-radius:15px; background: #ffffff; margin-bottom:1rem;">
                      ...
                      <div
                          ...
                          {{ strlen($item->title)  > 60 ? substr($item->title,0, 60)."..." : ucwords($item->title)}}
                      </div>
